...
def get_nearest_points_and_normals(vert, base_verts, base_faces):
    ...
    nearest_tri = nearest_tri.ravel().astype(np.int64)
    nearest_part = nearest_part.ravel().astype(np.int64)
    ...
    cl_tri_idxs = np.nonzero(nearest_part == 0)[0].astype(np.int32)
    cl_vrt_idxs = np.nonzero(nearest_part > 3)[0].astype(np.int32)
    cl_edg_idxs = np.nonzero((nearest_part <= 3) & (nearest_part > 0))[0].astype(np.int32)
...
def remove_interpenetration_fast(mesh, base, L=None):
    ...
    res = spsolve(A.T.dot(A), A.T.dot(b_mat))
